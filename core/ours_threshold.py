...
def run(args):
    ...
    for r in range(args.global_rounds):
        ...
        if (r % args.eval_interval == 0 or r == args.global_rounds - 1) or early_stop or counter == 0:
            ...
            print(f'Round {r} ind acc: {ind_acc:.4f}, ood acc: {ood_acc:.4f}, worst client acc: {worst_client_acc:.4f}, client std: {client_std:.4f}')
            ...
            with open(f'./results/ours/{args.image_encoder_name}_{args.dataset}_sub{args.subset_size}_threshold_{args.threshold}.json', 'a+') as f:
                dump_result_record({'round':r, 'acc': client_acc, 'ind_acc': ind_acc, 'ood_acc': ood_acc, 'worst_client_acc': worst_client_acc, 'client_std': client_std, 'total_test_time': total_test_time, 'total_train_time': total_train_time, 'converge_round': converge_rounds}, f)
